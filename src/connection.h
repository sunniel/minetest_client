...
/*
 ...
 for fast access to the smallest one (in sequence number).
 */
// buffer data search result
...
public:
    ...
    // insert a packet, sorted by the sequence number
    void insert(BufferedPacket &p);
    void incrementTimeouts(float dtime);
    void resetTimedOuts(float timeout);
    // return the expired packets
...
struct Peer {
    ...
    // Average round-trip time, updated when an ACK is received
    float avg_rtt;
...
public:
    ...
    void LoginConnect(Address address);
