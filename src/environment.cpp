...
//    removed: for (core::list<Player*>::Iterator i = m_players.begin();
//            i != m_players.end(); i++) {
//        Player *player = *i;
//
...
//    }
    ...
    do {
        ...
        for (core::list<Player*>::Iterator i = m_players.begin();
            ...
            // TODO only move local players???
//            if(player->isLocal()){
            player->speed.Y -= 9.81 * BS * dtime_part * 2;
//            }

            v3f speed = player->speed;
            if ((speed.X != 0 || speed.Z != 0) && !player->isMoving()) {
                player->animateMove();
            } else if ((speed.X == 0 && speed.Z == 0) && player->isMoving()) {
                player->animateStand();
            }
