...
public:
     ...
     */
    Client(scene::ISceneManager* smgr, video::SMaterial *materials, u16 peerId);
    ~Client();
    ...
    /*
     ...
     // Client environment simulation proceeds (in the main loop) with the following logic:
     1. receive block change and neighbor players from server;
     2. send position to server;
     3. clear block caches.
     */
    void step(float dtime);

    void ProcessData(u8 *data, u32 datasize, u16 peer_id);
    // Returns true if something was done
    // handle blockdata from server every 200ms
...
private:
    ...
    bool initState(float dtime);

    // send player position to server
    // m_con_mutex must be locked when calling these
    void sendPlayerState(float dtime);
