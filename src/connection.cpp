...
void IncomingSplitBuffer::insert(BufferedPacket &p, bool reliable) {
    ...
    if (reliable != sp->reliable)
        dout_con << "WARNING: reliable=" << reliable << " != sp->reliable="
...
void Connection::Connect(Address address) {
    ...
    /*
     * If the port is specified as zero, the service provider assigns a
     * unique port to the application from the dynamic client port range.
     */
    m_socket.Bind(0);

    // Send a dummy packet to server with peer_id = PEER_ID_NEW
    // To use the specified ID
//    m_peer_id = PEER_ID_NEW;
...
void Connection::LoginConnect(Address address) {
    core::map<u16, Peer*>::Node *node = m_peers.find(PEER_ID_LOGIN_SERVER);
    if (node != NULL) {
        throw ConnectionException("Already connected to a server");
    }
...
//    m_peer_id = PEER_ID_NEW;
    SharedBuffer<u8> data(0);
    Send(PEER_ID_LOGIN_SERVER, 0, data, true);
...
bool Connection::LoginServerConnected() {
    if (m_peers.size() != 1)
        return false;
...
SharedBuffer<u8> Channel::ProcessPacket(SharedBuffer<u8> packetdata,
    ...
    if (type == TYPE_CONTROL) { // update control info
        ...
        if (controltype == CONTROLTYPE_ACK) {
            ...
            try {
                // remove the data from the sending buffer (for reliable data transmission)
                BufferedPacket p = outgoing_reliables.popSeqnum(seqnum);

                // update peer information and configurations
                // Get round trip time
        ...
        con->PrintInfo();
        dout_con << "RETURNING TYPE_ORIGINAL to user" << std::endl;
        // Get the inside packet out and return it without header
        SharedBuffer<u8> payload(packetdata.getSize() - ORIGINAL_HEADER_SIZE);
        memcpy(*payload, &packetdata[ORIGINAL_HEADER_SIZE], payload.getSize());
        return payload;
    } else if (type == TYPE_SPLIT) { // cache the split data packets, until the complete data has been received
        ...
        /*
         * 1. respond ACK to the sender.
         * 2. process packet order: for future packet, re-cache it for future process; for old packet, just discard it.
         * 3. strip the header and recursively re-process it, which will go to the TYPE_CONTROL, TYPE_ORIGINAL, or
         * TYPE_SPLIT branch.
         */
        ...
        if (is_future_packet)
            ...
            dout_con << "RECUR";
        dout_con << " TYPE_RELIABLE seqnum=" << seqnum << " next="
...
SharedBuffer<u8> Channel::CheckIncomingBuffers(Connection *con, u16 &peer_id) {
    ...
    if (incoming_reliables.empty() == false) {
        if (firstseqnum == next_incoming_seqnum) {
            ...
            con->PrintInfo();
            dout_con << "UNBUFFERING TYPE_RELIABLE" << " seqnum=" << seqnum
...
u32 Connection::Receive(u16 &peer_id, u8 *data, u32 datasize) {
    ...
    /*
     * for loop ends when there is no incoming packet (by throwing a
     * NoIncomingDataException) from the specified peer
     */
    for (;;) {
        try {
            /*
            ...
            if (channelnum > CHANNEL_COUNT - 1) {
                PrintInfo();
                dout_con << "Receive(): Invalid channel " << channelnum
                        << std::endl;
             ...
             */
            // extract the payload by making a new SharedBuffer from the data without the base headers
...
void Connection::RunTimeouts(float dtime) {
    ...
    for (; j.atEnd() == false; j++) {
        ...
        if (peer->timeout_counter > 30.0) {
            PrintInfo();
            dout_con << "RunTimeouts(): Peer " << peer->id << " has timeouted."
                    << std::endl;
        ...
        for (u16 i = 0; i < CHANNEL_COUNT; i++) {
            ...
            for (; j != timed_outs.end(); j++) {
                ...
                PrintInfo();
                dout_con << "RE-SENDING timed-out RELIABLE: " << "peer_id="
