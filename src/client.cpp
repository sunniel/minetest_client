...
Client::Client(scene::ISceneManager* smgr, video::SMaterial *materials,
...

    {
        ...
        m_env.getMap().load(
                properties::getString("directory", "node_distribution", "").c_str());
        ...
        f32 y = BS * 2 + BS * 10;
        // initial with a random location
        int MAX_LENGTH = (float) properties::getDouble("map", "length_bs", 100);
        int MAX_WIDTH = (float) properties::getDouble("map", "width_bs", 100);
        float x = static_cast<float>(rand() % MAX_LENGTH);
        float z = static_cast<float>(rand() % MAX_WIDTH);
        player->setPosition(v3f(x, y, z));
...
void Client::step(float dtime) {
    // handle client-server communication for add/remove node and update player position
    ReceiveAll();

    bool connected = false;
    {
        JMutexAutoLock lock(m_con_mutex);
        // currently, connection timeout to Mesh is not applied
...
void Client::ProcessData(u8 *data, u32 datasize, u16 peer_id) {
    ...
    if (command == TOCLIENT_REMOVENODE) {
        ...
        {
            JMutexAutoLock lock(m_con_mutex);
            // peer id is retrieved from Connection, which is further replied by server
        ...
        { //envlock
            ...
            u32 player_count = (datasize - 2) / (2 + 12 + 12 + 12 + 2 + 2);
            u32 start = 2;
            for (u32 i = 0; i < player_count; i++) {
                ...
                if (player == NULL) {
                    ...
                    dout_client << "Client: Adding new player " << peer_id
                            << std::endl;
                    std::cout << "Client: Adding new player " << peer_id
                            << std::endl;
                    isNew = true;
                }
                ...
                v3s32 rs = readV3S32(&data[start + 2 + 12 + 12]);
                v3f position((f32) ps.X / 100., (f32) ps.Y / 100.,
                        (f32) ps.Z / 100.);
                v3f speed((f32) ss.X / 100., (f32) ss.Y / 100.,
                        (f32) ss.Z / 100.);
                v3f rotatoin((f32) rs.X / 100., (f32) rs.Y / 100.,
                        (f32) rs.Z / 100.);
//                player->setPosition(position);
                player->updatePosition(position);
                player->speed = speed;
                player->setRotation(rotatoin);

                // update state
                u16 oldskin = player->skin;
...
//                if (!player->isLocal()) {
//                    cout << "neighbor rotation: " << rotatoin.X << ", "
//                            << rotatoin.Y << ", " << rotatoin.Z << endl;
//                }
    ...
    } else if (command == TOCLIENT_RMPLAYER) {
        if (datasize < 4)
            return;

        std::cout << "received remove player from the Mesh" << endl;
        ...
        if (player != NULL) {
            ...
            dout_client << "player to remove found " << std::endl;
            std::cout << "remove player " << peer_id << std::endl;

            m_env.removePlayer(player);
        } else {
            std::cout << "player doesn't exist" << endl;
        }
    } else if (command == TOCLIENT_INIT) {
        if (datasize < 18)
            return;

        cout << "received init data from the Mesh" << endl;

        Player* player = getLocalPlayer();
        v3s32 ps = readV3S32(&data[2]);
        u16 skin = readU16(&data[2 + 12]);
        u16 gear = readU16(&data[2 + 12 + 2]);
        v3f position((f32) ps.X / 100., (f32) ps.Y / 100., (f32) ps.Z / 100.);
        player->setPosition(position);
        player->updateMesh(m_smgr, skin, gear);
        init = true;

    } else if (command == TOCLIENT_INIT_EMPTY) {
        init = true;

        cout << "received empty init from the Mesh" << endl;
...
// handle client-server communication for slow commands
...
bool Client::initState(float dtime) {
    if (init) {
        return true;
    } else {
        // query server at reasonable intervals (5s)
        ...
        cout << "Send Init request to the Mesh" << endl;

        SharedBuffer<u8> data(2);
        writeU16(&data[0], TOSERVER_INIT);
        // init state communication through channel 1
        Send(1, data, true);
    }
    return false;
}

void Client::sendPlayerState(float dtime) {
    JMutexAutoLock envlock(m_env_mutex);
    ...
    // Update at reasonable intervals (2s)
    static float counter = 0.0;
    counter += dtime;
    if (counter < 2)
        return;
    ...
    // send player state
    writeU16(&data[2 + 12 + 12 + 12], myplayer->skin);
    writeU16(&data[2 + 12 + 12 + 12 + 2], myplayer->gear);
